...
--------------------------------------------------------------------------------
-- 1. Identification
--    Filename          :  cryptada-digests-message_digests-blake2b.adb
...
--------------------------------------------------------------------------------
-- 2. Purpose:
--    Implements the BLAKE2b message digest algorithm.
--------------------------------------------------------------------------------
-- 3. Revision history
--    Ver   When     Who   Why
--    ----- -------- ----- -----------------------------------------------------
...
package body CryptAda.Digests.Message_Digests.BLAKE2b is
   ...
   --[BLAKE2b_Parameter_Block_Bytes]--------------------------------------------
   -- Size in bytes of BLAKE2b parameter block.
   -----------------------------------------------------------------------------

   BLAKE2b_Parameter_Block_Bytes : constant Positive := 64;

   --[BLAKE2b_Parameter_Block_Words]--------------------------------------------
   -- Size in words of BLAKE2b parameter block.
   -----------------------------------------------------------------------------

   BLAKE2b_Parameter_Block_Words : constant Positive := BLAKE2b_Parameter_Block_Bytes / BLAKE2b_Word_Bytes;

   --[BLAKE2b_Block_Words]------------------------------------------------------
   -- Size in words of BLAKE2b block.
   -----------------------------------------------------------------------------

   BLAKE2b_Block_Words           : constant Positive := BLAKE2b_Block_Bytes / BLAKE2b_Word_Bytes;

   --[BLAKE2b_Sigma]------------------------------------------------------------
   -- BLAKE2b permutation table.
   -----------------------------------------------------------------------------

   BLAKE2b_Sigma                 : constant array(Byte range 0 .. 11, Byte range 0 .. 15) of Byte :=
      (
   ...
   --[BLAKE2b_Parameter_Block]--------------------------------------------------
   -- BLAKE2b parameter block type.
   -----------------------------------------------------------------------------

   subtype BLAKE2b_Parameter_Block is Byte_Array(1 .. BLAKE2b_Parameter_Block_Bytes);

   --[BLAKE2b_Packed_PB]--------------------------------------------------------
   -- Type for packed parameter block.
   -----------------------------------------------------------------------------

   subtype BLAKE2b_Packed_PB is Eight_Bytes_Array(1 .. BLAKE2b_Parameter_Block_Words);

   --[BLAKE2b_Packed_Block]-----------------------------------------------------
   -- Packed block type.
   -----------------------------------------------------------------------------

   subtype BLAKE2b_Packed_Block is Eight_Bytes_Array(1 .. BLAKE2b_Block_Words);

   --[BLAKE2b_Unpacked_State]---------------------------------------------------
   -- Unpacked state type.
   -----------------------------------------------------------------------------

   subtype BLAKE2b_Unpacked_State is Byte_Array(1 .. BLAKE2b_State_Bytes);

   --[BLAKE2b_Working_Vector]---------------------------------------------------
   -- BLAKE2b working vector.
   -----------------------------------------------------------------------------

   subtype BLAKE2b_Working_Vector is Eight_Bytes_Array(1 .. 16);
   ...
   function    Get_Hash_Bytes(
                  From_List      : in     List)
      return   BLAKE2b_Hash_Bytes;

   --[Get_Key]------------------------------------------------------------------

   procedure   Get_Key(
                  From_List      : in     List;
                  Key_Bytes      :    out BLAKE2b_Key_Bytes;
                  Key            :    out BLAKE2b_Key);
   ...
   procedure   Get_Parameters(
                  From_List      : in     List;
                  Hash_Bytes     :    out BLAKE2b_Hash_Bytes;
                  Key_Bytes      :    out BLAKE2b_Key_Bytes;
                  Key            :    out BLAKE2b_Key;
                  Salt           :    out BLAKE2b_Salt;
                  Personal       :    out BLAKE2b_Personal);
   ...
   function    Set_Parameter_Block(
                  ...
                  Salt           : in     BLAKE2b_Salt;
                  Personal       : in     BLAKE2b_Personal)
      return   BLAKE2b_Packed_PB;
   ...
   function    Pack_Block(
                  Block          : in     BLAKE2b_Block)
      return   BLAKE2b_Packed_Block;
   pragma Inline(Pack_Block);

   --[Pack_Parameter_Block]-----------------------------------------------------

   function    Pack_Parameter_Block(
                  P_Block        : in     BLAKE2b_Parameter_Block)
      return   BLAKE2b_Packed_PB;
   ...
   function    Unpack_State(
                  State          : in     BLAKE2b_State)
      return   BLAKE2b_Unpacked_State;
   pragma Inline(Unpack_State);

   --[BLAKE2b specific operations]----------------------------------------------

   --[G]------------------------------------------------------------------------

   procedure   G(
                  PB             : in     BLAKE2b_Packed_Block;
                  Round          : in     Byte;
                  I              : in     Byte;
                  A              : in out Eight_Bytes;
                  B              : in out Eight_Bytes;
                  C              : in out Eight_Bytes;
                  D              : in out Eight_Bytes);
   pragma Inline(G);
   ...
   procedure   Round(
                  R              : in     Byte;
                  PB             : in     BLAKE2b_Packed_Block;
                  V              : in out BLAKE2b_Working_Vector);
   pragma Inline(Round);

   --[Compress]-----------------------------------------------------------------

   procedure   Compress(
                  State          : in out BLAKE2b_State;
                  BCount         : in     BLAKE2b_BCount;
                  FFLags         : in     BLAKE2b_FFLags;
                  B              : in     BLAKE2b_Block);
   ...
   begin
      ...
      if HB in BLAKE2b_Hash_Bytes'Range then
         return HB;
      else
         Raise_Exception(
      ...
      when X: others =>
         Raise_Exception(
            ...
            "Caught exception: " &
               ...
               ", with message: """ &
               Exception_Message(X) &
               """. When obtaining BLAKE2b 'Hash_Bytes' parameter");
   end Get_Hash_Bytes;
   ...
   begin
      ...
      if KB in BLAKE2b_Key_Bytes'Range then
         Key_Bytes := KB;
      else
         Raise_Exception(
            CryptAda_Bad_Argument_Error'Identity,
      ...
      when X: others =>
         Raise_Exception(
            ...
            "Caught exception: " &
               ...
               ", with message: """ &
               Exception_Message(X) &
               """. When obtaining BLAKE2b 'Key' parameter");
   end Get_Key;
   ...
   begin
      if SB'Length /= BLAKE2b_Salt_Bytes then
         Raise_Exception(
            CryptAda_Bad_Argument_Error'Identity,
            "Invalid 'Salt' length");
      else
      ...
      when X: others =>
         Raise_Exception(
            ...
            "Caught exception: " &
               ...
               ", with message: """ &
               Exception_Message(X) &
               """. When obtaining BLAKE2b 'Salt' parameter");
   end Get_Salt;
   ...
   begin
      if PB'Length /= BLAKE2b_Personal_Bytes then
         Raise_Exception(
            CryptAda_Bad_Argument_Error'Identity,
      ...
      when X: others =>
         Raise_Exception(
            ...
            "Caught exception: " &
               ...
               ", with message: """ &
               Exception_Message(X) &
               """. When obtaining BLAKE2b 'Personal' parameter");
   end Get_Personal;
   ...
   begin
      ...
      if Get_List_Kind(From_List) = Empty then
         ...
         Hash_Bytes  := BLAKE2b_Default_Hash_Bytes;
         Key_Bytes   := BLAKE2b_No_Key;
         Key         := (others => 16#00#);
         Salt        := BLAKE2b_Default_Salt;
         Personal    := BLAKE2b_Default_Personal;
      ...
      when X: others =>
         Raise_Exception(
            ...
            "Caught exception: " &
               ...
               ", with message: """ &
               Exception_Message(X) &
               """. When parsing BLAKE2b parameter list");
   end Get_Parameters;
   ...
   begin
      ...
      Private_Set_Hash_Size(Object, BLAKE2b_Default_Hash_Bytes);

      Object.all.State        := BLAKE2b_Initial_State;
      Object.all.BCount       := BLAKE2b_Zero_BCount;
      Object.all.FFlags       := (others => 16#0000000000000000#);
   ...
   function    Set_Parameter_Block(
      ...
      return   BLAKE2b_Packed_PB
   is
      PB             : BLAKE2b_Parameter_Block := (others => 16#00#);
   begin
      ...
      PB(13 .. 16)   := (others => 16#00#);  -- Xof length set to 0 in sequential mode.      
      PB(17)         := 16#00#;              -- Node depth set to 0 in sequential mode.
      PB(18)         := 16#00#;              -- Inner length set to 0 in sequential mode.
      PB(19 .. 32)   := (others => 16#00#);  -- Reserved (14 bytes).
      PB(33 .. 48)   := Salt;
      PB(49 .. 64)   := Personal;
   ...
   function    Pack_Block(
      ...
      return   BLAKE2b_Packed_Block
   is
      PB             : BLAKE2b_Packed_Block  := (others => 16#0000000000000000#);
      J              : Positive              := Block'First;
   begin
      for I in PB'Range loop
         PB(I) := Pack(Block(J .. J + BLAKE2b_Word_Bytes - 1), Little_Endian);
         J := J + BLAKE2b_Word_Bytes;
      end loop;
   ...
   function    Pack_Parameter_Block(
      ...
      return   BLAKE2b_Packed_PB
   is
      PB             : BLAKE2b_Packed_PB     := (others => 16#000000000000#);
      J              : Positive              := P_Block'First;
   begin
      for I in PB'Range loop
         PB(I) := Pack(P_Block(J .. J + BLAKE2b_Word_Bytes - 1), Little_Endian);
         J := J + BLAKE2b_Word_Bytes;
      end loop;
   ...
   function    Unpack_State(
      ...
      return   BLAKE2b_Unpacked_State
   is
      US             : BLAKE2b_Unpacked_State   := (others => 16#00#);
      J              : Positive                 := US'First;
   begin
      for I in State'Range loop
         US(J .. J + BLAKE2b_Word_Bytes - 1) := Unpack(State(I), Little_Endian);
         J := J + BLAKE2b_Word_Bytes;
      end loop;
   ...
   is
      K              : Positive;
   begin
      K  := 1 + Natural(BLAKE2b_Sigma(Round, 2 * I));
      A  := A + B + PB(K);
      D  := Rotate_Right((D xor A), 32);
      C  := C + D;
      B  := Rotate_Right((B xor C), 24);

      K  := 1 + Natural(BLAKE2b_Sigma(Round, 1 + (2 * I)));
      A  := A + B + PB(K);
   ...
   procedure   Compress(
                  ...
                  B              : in     BLAKE2b_Block)
   is
      PB             : constant BLAKE2b_Packed_Block := Pack_Block(B);
      V              : BLAKE2b_Working_Vector := (others => 16#0000000000000000#);
      J              : Positive;
   begin
      -- Initialize working vector.

      V( 1 ..  8) := State;
      V( 9 .. 12) := BLAKE2b_Initial_State(1 .. 4);
      V(13)       := BCount(1) xor BLAKE2b_Initial_State(5);
      V(14)       := BCount(2) xor BLAKE2b_Initial_State(6);
      V(15)       := FFlags(1) xor BLAKE2b_Initial_State(7);
      V(16)       := FFlags(2) xor BLAKE2b_Initial_State(8);

      -- Perform the 12 rounds.
   ...
   is
      P           : BLAKE2b_Digest_Ptr;
   begin
      P := new BLAKE2b_Digest'(Message_Digest with
                                    Id          => MD_BLAKE2b,
                                    State       => BLAKE2b_Initial_State,
                                    BCount      => BLAKE2b_Zero_BCount,
                                    FFlags      => (others => 16#0000000000000000#),
                                    Last_Node   => False,
                                    BIB         => 0,
                                    Buffer      => (others => 16#00#));
   ...
   exception
      when others =>
         Raise_Exception(
            ...
            "Error when allocating BLAKE2b_Digest object");
   ...
   begin
      ...
      The_Digest.State        := BLAKE2b_Initial_State;
      The_Digest.BCount       := BLAKE2b_Zero_BCount;
      The_Digest.FFlags       := (others => 16#0000000000000000#);
   ...
   is
   begin
      Digest_Start(The_Digest, BLAKE2b_Default_Hash_Bytes);
   end Digest_Start;
   ...
   procedure   Digest_Start(
                  ...
                  Parameters     : in     List)
   is
      HB             : BLAKE2b_Hash_Bytes;
      KB             : BLAKE2b_Key_Bytes;
      K              : BLAKE2b_Key;
      S              : BLAKE2b_Salt;
      P              : BLAKE2b_Personal;
   begin
   ...
   is
      ...
      Fill           : constant Natural := BLAKE2b_Block_Bytes - Left;
   ...
   begin
      ...
      -- BLAKE2b_Block_Bytes, the entire last block is buffered because the 
      ...
      if L > 0 then
         ...
         if L > Fill then
            ...
            The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. BLAKE2b_Block_Bytes) := The_Bytes(I .. I + Fill - 1);

            -- Update internal counter.

            The_Digest.all.BCount(1) := The_Digest.all.BCount(1) + Eight_Bytes(BLAKE2b_Block_Bytes);

            if The_Digest.all.BCount(1) < Eight_Bytes(BLAKE2b_Block_Bytes) then
            ...
            while L > BLAKE2b_Block_Bytes loop
               -- Update internal counter.
               ...
               Compress(
                  ...
                  The_Digest.all.BCount,
                  The_Digest.all.FFlags,
                  The_Bytes(I .. I + BLAKE2b_Block_Bytes - 1));

               -- Update index over The_Bytes, decrease number of bytes left.

               I := I + BLAKE2b_Block_Bytes;
               L := L - BLAKE2b_Block_Bytes;
            end loop;
         end if;
   ...
   begin
      ...
      The_Digest.all.BCount(1) := The_Digest.all.BCount(1) + Eight_Bytes(The_Digest.all.BIB);

      if The_Digest.all.BCount(1) < Eight_Bytes(The_Digest.all.BIB) then
         The_Digest.all.BCount(2) := The_Digest.all.BCount(2) + 1;
      end if;
      ...
      if The_Digest.all.Last_Node then
         The_Digest.all.FFlags(2)   := 16#FFFF_FFFF_FFFF_FFFF#;
      end if;

      The_Digest.all.FFLags(1)   := 16#FFFF_FFFF_FFFF_FFFF#;
   ...
   is
      PPB            : BLAKE2b_Packed_PB := (others => 16#00000000#);
   begin
      -- Initialize object.
      ...
      PPB := Set_Parameter_Block(Hash_Bytes, BLAKE2b_No_Key, Salt, Personal);
