...
--------------------------------------------------------------------------------
-- 1. Identification
--    Filename          :  cryptada-digests-message_digests-ripemd_160.adb
...
package body CryptAda.Digests.Message_Digests.RIPEMD_160 is
   ...
   --[Bit_Counter_Offset]-------------------------------------------------------
   -- Index of the first byte of the bit counter inside the RIPEMD_160_Block.
   -- The 8 byte counter will occupy the last 8 positions of the last block.
   -----------------------------------------------------------------------------

   Bit_Counter_Offset      : constant Positive := 1 + RIPEMD_160_Block_Bytes - 8;

   --[RIPEMD_160_Block_Words]---------------------------------------------------
   -- Size in words of MD5 block.
   -----------------------------------------------------------------------------

   RIPEMD_160_Block_Words  : constant Positive := RIPEMD_160_Block_Bytes / 4;

   --[RIPEMD_160_Pad]----------------------------------------------------------
   -- Pad block.
   -----------------------------------------------------------------------------

   RIPEMD_160_Pad          : constant RIPEMD_160_Block := (1 => 16#80#, others => 16#00#);

   -----------------------------------------------------------------------------
   --[Types]--------------------------------------------------------------------
   -----------------------------------------------------------------------------

   --[RIPEMD_160_Packed_Block]--------------------------------------------------
   -- Packed block type.
   -----------------------------------------------------------------------------

   subtype RIPEMD_160_Packed_Block is Four_Bytes_Array(1 .. RIPEMD_160_Block_Words);

   --[RIPEMD_160_Unpacked_State]------------------------------------------------
   -- Unpacked state type.
   -----------------------------------------------------------------------------

   subtype RIPEMD_160_Unpacked_State is Byte_Array(1 .. RIPEMD_160_State_Bytes);
   ...
   procedure   Initialize_Object(
                  Object         : access RIPEMD_160_Digest);
   ...
   function    Pack_Block(
                  The_Block      : in     RIPEMD_160_Block)
      return   RIPEMD_160_Packed_Block;
   pragma Inline(Pack_Block);

   --[Unpack_State]-------------------------------------------------------------

   function    Unpack_State(
                  The_State      : in     RIPEMD_160_State)
      return   RIPEMD_160_Unpacked_State;
   ...
   procedure   Transform(
                  State          : in out RIPEMD_160_State;
                  Block          : in     RIPEMD_160_Block);
   ...
   begin
      ...
      Object.all.State     := RIPEMD_160_Initial_State;
      Object.all.BIB       := 0;
   ...
   procedure   Transform(
                  ...
                  Block          : in     RIPEMD_160_Block)
   is
      X              : constant RIPEMD_160_Packed_Block := Pack_Block(Block);
   ...
   begin
      ...
      DDD      := State(2) + CC + DDD;
      State(2) := State(3) + DD + EEE;
      State(3) := State(4) + EE + AAA;
      State(4) := State(5) + AA + BBB;
      State(5) := State(1) + BB + CCC;
      State(1) := DDD;
   end Transform;
   ...
   is
      P           : RIPEMD_160_Digest_Ptr;
   begin
      P := new RIPEMD_160_Digest'(Message_Digest with
                                    Id          => MD_RIPEMD_160,
                                    State       => RIPEMD_160_Initial_State,
                                    BIB         => 0,
                                    Buffer      => (others => 16#00#));
      Private_Initialize_Digest(
         P.all,
         RIPEMD_160_State_Bytes,
         RIPEMD_160_Block_Bytes,
   ...
   exception
      when others =>
         Raise_Exception(
            ...
            "Error when allocating RIPEMD_160_Digest object");
   ...
   begin
      ...
      The_Digest.State        := RIPEMD_160_Initial_State;
      The_Digest.BIB          := 0;
   ...
   procedure   Digest_Start(
                  The_Digest     : access RIPEMD_160_Digest)
   is
   begin
   ...
   pragma Warnings (On, "formal parameter ""Parameters"" is not referenced");
      -- Parameters is ignored because RIPEMD-160 does not expect any parameter.
   begin
   ...
   is
      ...
      Chunks         : Natural            := TB / RIPEMD_160_Block_Bytes;
      New_BIB        : constant Natural   := TB mod RIPEMD_160_Block_Bytes;
      I              : Natural            := The_Bytes'First;
      To_Copy        : Natural            := 0;
   begin
      -- Data is processed in chunks of RIPEMD_160_Block_Bytes bytes.
      ...
      if Chunks > 0 then
         ...
         if The_Digest.all.BIB > 0 then
            To_Copy := RIPEMD_160_Block_Bytes - The_Digest.all.BIB;
            The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. RIPEMD_160_Block_Bytes) :=
         ...
         while Chunks > 0 loop
            Transform(
               ...
               The_Bytes(I .. I + RIPEMD_160_Block_Bytes - 1));

            -- Update index over The_Bytes, decrease number of chunks.

            I := I + RIPEMD_160_Block_Bytes;
            Chunks := Chunks - 1;
         end loop;
      end if;
   ...
   is
      ...
      To_Pad         : constant Natural := RIPEMD_160_Block_Bytes - The_Digest.BIB;
   begin
      -- Pad buffer.

      if To_Pad > 0 then
         The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. RIPEMD_160_Block_Bytes) := RIPEMD_160_Pad(1 .. To_Pad);
      end if;
      ...
      The_Digest.all.Buffer(Bit_Counter_Offset .. RIPEMD_160_Block_Bytes) := UC(1 .. 8);
      Transform(The_Digest.all.State, The_Digest.all.Buffer);
...
end CryptAda.Digests.Message_Digests.RIPEMD_160;
