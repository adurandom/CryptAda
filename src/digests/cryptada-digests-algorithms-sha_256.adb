...
--------------------------------------------------------------------------------
-- 1. Identification
--    Filename          :  cryptada-digests-algorithms-sha_256.adb
...
package body CryptAda.Digests.Algorithms.SHA_256 is
   ...
   is
      ...
      Chunks         : Natural := Tot_Bytes / SHA_256_Block_Bytes;
      New_BIB        : constant Natural := Tot_Bytes mod SHA_256_Block_Bytes;
   ...
   begin
      ...
      if Chunks > 0 then
         ...
         if The_Digest.BIB > 0 then
            To_Copy := SHA_256_Block_Bytes - The_Digest.BIB;
            The_Digest.Buffer(The_Digest.BIB + 1 .. SHA_256_Block_Bytes) := The_Bytes(I .. I + To_Copy - 1);
         ...
         while Chunks > 0 loop
            Transform(The_Digest.State, The_Bytes(I .. I + SHA_256_Block_Bytes - 1));
   ...
   is
      ...
      To_Pad         : constant Natural := SHA_256_Block_Bytes - The_Digest.BIB;
   begin

      -- Pad message.

      if To_Pad > 0 then
         The_Digest.Buffer(The_Digest.BIB + 1 .. SHA_256_Block_Bytes) := SHA_256_Pad(1 .. To_Pad);
      end if;
      ...
      The_Digest.Buffer(Bit_Counter_Offset .. SHA_256_Block_Bytes) := UC(9 .. 16);
      Transform(The_Digest.State, The_Digest.Buffer);
   ...
   is
   begin
      The_Digest.Algorithm_Id := MD_SHA_256;
      The_Digest.State_Size   := SHA_256_State_Bytes;
      The_Digest.Block_Size   := SHA_256_Block_Bytes;
      The_Digest.Hash_Size    := SHA_256_Hash_Bytes;
