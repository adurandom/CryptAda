...
--------------------------------------------------------------------------------
-- 1. Identification
--    Filename          :  cryptada-digests-message_digests-sha_512.adb
...
package body CryptAda.Digests.Message_Digests.SHA_512 is
   ...
   begin
      ...
      Object.all.State     := SHA_512_Initial_State;
      Object.all.BIB       := 0;
   ...
   is
      P           : SHA_512_Digest_Ptr;
   begin
      P := new SHA_512_Digest'(Message_Digest with
                                 Id          => MD_SHA_512,
                                 State       => SHA_512_Initial_State,
                                 BIB         => 0,
                                 Buffer      => (others => 16#00#));
   ...
   exception
      when others =>
         Raise_Exception(
            ...
            "Error when allocating SHA_512_Digest object");
   ...
   pragma Warnings (On, "formal parameter ""Parameters"" is not referenced");
      -- Parameters is ignored because SHA-512 does not expect any parameter.
   begin
   ...
   is
      ...
      Chunks         : Natural            := TB / SHA_512_Block_Bytes;
      New_BIB        : constant Natural   := TB mod SHA_512_Block_Bytes;
   ...
   begin
      ...
      if Chunks > 0 then
         ...
         if The_Digest.all.BIB > 0 then
            To_Copy := SHA_512_Block_Bytes - The_Digest.all.BIB;
            The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. SHA_512_Block_Bytes) := 
         ...
         while Chunks > 0 loop
            Transform(The_Digest.all.State, The_Bytes(I .. I + SHA_512_Block_Bytes - 1));
   ...
   is
      ...
      To_Pad         : constant Natural := SHA_512_Block_Bytes - The_Digest.all.BIB;
   begin
      -- Pad message.

      if To_Pad > 0 then
         The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. SHA_512_Block_Bytes) := 
            SHA_512_Pad(1 .. To_Pad);
      end if;
      ...
      The_Digest.all.Buffer(Bit_Counter_Offset .. SHA_512_Block_Bytes) := UC;
      Transform(The_Digest.all.State, The_Digest.all.Buffer);
