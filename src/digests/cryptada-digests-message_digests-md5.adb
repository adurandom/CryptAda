...
--------------------------------------------------------------------------------
-- 1. Identification
--    Filename          :  cryptada-digests-message_digests-md5.adb
...
package body CryptAda.Digests.Message_Digests.MD5 is
   ...
   begin
      ...
      Object.all.State     := MD5_Initial_State;
      Object.all.BIB       := 0;
   ...
   is
      P           : MD5_Digest_Ptr;
   begin
      P := new MD5_Digest'(Message_Digest with
                                 Id          => MD_MD5,
   ...
   exception
      when others =>
         Raise_Exception(
            ...
            "Error when allocating MD5_Digest object");
   ...
   pragma Warnings (On, "formal parameter ""Parameters"" is not referenced");
      -- Parameters is ignored because MD5 does not expect any parameter.
   begin
   ...
   is
      ...
      Chunks         : Natural            := TB / MD5_Block_Bytes;
      New_BIB        : constant Natural   := TB mod MD5_Block_Bytes;
   ...
   begin
      ...
      if Chunks > 0 then
         -- At least one chunk of data must be processed. Check if there are any
         -- bytes in internal buffer.

         if The_Digest.all.BIB > 0 then
            -- There are some input bytes buffered in The_Digest, fill the buffer
            -- with data from The_Bytes and transform.

            To_Copy := MD5_Block_Bytes - The_Digest.all.BIB;
            The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. MD5_Block_Bytes) :=
         ...
         while Chunks > 0 loop
            Transform(
               ...
               The_Bytes(I .. I + MD5_Block_Bytes - 1));
   ...
   is
      ...
      To_Pad         : constant Natural := MD5_Block_Bytes - The_Digest.all.BIB;
   begin
      -- Pad buffer.
      ...
      The_Digest.all.Buffer(Bit_Counter_Offset .. MD5_Block_Bytes) := UC(1 .. 8);
      Transform(The_Digest.all.State, The_Digest.all.Buffer);
...
end CryptAda.Digests.Message_Digests.MD5;
