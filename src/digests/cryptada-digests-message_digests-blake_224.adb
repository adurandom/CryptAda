...
package body CryptAda.Digests.Message_Digests.BLAKE_224 is
   ...
   --[BLAKE_224_Block_Bits]-----------------------------------------------------
   -- Size in bits of a BLAKE-224 block.
   -----------------------------------------------------------------------------

   BLAKE_224_Block_Bits    : constant Four_Bytes := Four_Bytes(8 * BLAKE_224_Block_Bytes);
   ...
   BLAKE_224_Constants     : constant Four_Bytes_Array(1 .. 16) :=
      (
   ...
   is
      ...
      V              : BLAKE_224_Working_Vector := (others => 16#00000000#);
   begin
      -- Initialization.
   ...
   begin
      ...
      if Chunks > 0 then
         ...
         if The_Digest.BIB > 0 then
            ...
            The_Digest.all.BCount(1) := The_Digest.all.BCount(1) + BLAKE_224_Block_Bits;
