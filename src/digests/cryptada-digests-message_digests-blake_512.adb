...
--------------------------------------------------------------------------------
-- 1. Identification
--    Filename          :  cryptada-digests-message_digests-blake_512.adb
...
package body CryptAda.Digests.Message_Digests.BLAKE_512 is
   ...
   --[Bit_Counter_Offset]-------------------------------------------------------
   -- Index of the first byte of the bit counter inside the BLAKE_512_Block. The
   -- 16 byte counter will occupy the last 16 positions of the last block.
   -----------------------------------------------------------------------------

   Bit_Counter_Offset      : constant Positive := 1 + BLAKE_512_Block_Bytes - 16;

   --[BLAKE_512_Block_Bits]-----------------------------------------------------
   -- Size in bits of a BLAKE-512 block.
   -----------------------------------------------------------------------------

   BLAKE_512_Block_Bits    : constant Eight_Bytes := Eight_Bytes(8 * BLAKE_512_Block_Bytes);
   
   --[BLAKE_512_Block_Words]----------------------------------------------------
   -- Size in words of BLAKE-512 block.
   -----------------------------------------------------------------------------

   BLAKE_512_Block_Words   : constant Positive := BLAKE_512_Block_Bytes / BLAKE_512_Word_Bytes;

   --[BLAKE_512_Rounds]---------------------------------------------------------
   -- Number of BLAKE-512 rounds
   -----------------------------------------------------------------------------

   BLAKE_512_Rounds        : constant Positive := 16;

   --[BLAKE_512_Padding]--------------------------------------------------------
   -- Array for padding.
   -----------------------------------------------------------------------------

   BLAKE_512_Padding       : constant BLAKE_512_Block := (1 => 16#80#, others => 16#00#);
   
   --[BLAKE_512_Constants]------------------------------------------------------
   -- The BLAKE-512 constants
   -----------------------------------------------------------------------------

   BLAKE_512_Constants     : constant Eight_Bytes_Array(1 .. 16) :=
      (
   ...
   --[BLAKE_512_Sigma]----------------------------------------------------------
   -- The BLAKE-512 permutations
   -----------------------------------------------------------------------------

   BLAKE_512_Sigma         : constant array(Byte range 0 .. 15, Byte range 0.. 15) of Byte :=
      (
   ...
   --[BLAKE_512_Packed_Block]---------------------------------------------------
   -- Packed block type.
   -----------------------------------------------------------------------------

   subtype BLAKE_512_Packed_Block is Eight_Bytes_Array(1 .. BLAKE_512_Block_Words);

   --[BLAKE_512_Unpacked_State]-------------------------------------------------
   -- Unpacked state type.
   -----------------------------------------------------------------------------

   subtype BLAKE_512_Unpacked_State is Byte_Array(1 .. BLAKE_512_State_Bytes);

   --[BLAKE_512_Working_Vector]-------------------------------------------------
   -- BLAKE-512 working vector.
   -----------------------------------------------------------------------------

   subtype BLAKE_512_Working_Vector is Eight_Bytes_Array(1 .. 16);
   ...
   procedure   Get_Parameters(
                  From_List      : in     List;
                  Salt           :    out BLAKE_512_Salt);

   --[Initialize_Object]--------------------------------------------------------

   procedure   Initialize_Object(
                  Object         : access BLAKE_512_Digest);
   pragma Inline(Initialize_Object);

   --[Pack_Block]---------------------------------------------------------------

   function    Pack_Block(
                  The_Block      : in     BLAKE_512_Block)
      return   BLAKE_512_Packed_Block;
   pragma Inline(Pack_Block);

   --[Pack_Salt]----------------------------------------------------------------

   function    Pack_Salt(
                  The_Salt       : in     BLAKE_512_Salt)
      return   BLAKE_512_Packed_Salt;
   pragma Inline(Pack_Salt);

   --[Unpack_Salt]--------------------------------------------------------------

   function    Unpack_Salt(
                  The_Salt       : in     BLAKE_512_Packed_Salt)
      return   BLAKE_512_Salt;
   ...
   function    Unpack_State(
                  The_State      : in     BLAKE_512_State)
      return   BLAKE_512_Unpacked_State;
   pragma Inline(Unpack_State);

   --[G]------------------------------------------------------------------------

   procedure   G(
                  V              : in out BLAKE_512_Working_Vector;
                  X              : in     BLAKE_512_Packed_Block;
                  A              : in     Positive;
                  B              : in     Positive;
                  C              : in     Positive;
                  D              : in     Positive;
   ...
   procedure   Transform(
                  State          : in out BLAKE_512_State;
                  Block          : in     BLAKE_512_Block;
                  Salt           : in     BLAKE_512_Packed_Salt;
                  BCount         : in     BLAKE_512_BCount;
   ...
   begin
      if SB'Length /= BLAKE_512_Salt_Bytes then
         Raise_Exception(
      ...
      when X: others =>
         Raise_Exception(
            ...
            "Caught exception: " &
               ...
               ", with message: """ &
               Exception_Message(X) &
               """. When obtaining BLAKE-512 'Salt' parameter");
   end Get_Salt;
      ...
      when X: others =>
         Raise_Exception(
            ...
            "Caught exception: " &
               ...
               ", with message: """ &
               Exception_Message(X) &
               """. When parsing BLAKE-512 parameter list");
   end Get_Parameters;
   ...
   begin
      ...
      Object.all.Salt   := BLAKE_512_Default_Packed_Salt;
      Object.all.State  := BLAKE_512_Initial_State;
      Object.all.BCount := BLAKE_512_Zero_BCount;
      Object.all.BIB    := 0;
   ...
   function    Pack_Block(
      ...
      return   BLAKE_512_Packed_Block
   is
      PB             : BLAKE_512_Packed_Block   := (others => 16#0000000000000000#);
      J              : Positive                 := The_Block'First;
   begin
      for I in PB'Range loop
         PB(I) := Pack(The_Block(J .. J + BLAKE_512_Word_Bytes - 1), Big_Endian);
         J := J + BLAKE_512_Word_Bytes;
      end loop;
   ...
   function    Pack_Salt(
      ...
      return   BLAKE_512_Packed_Salt
   is
      PS             : BLAKE_512_Packed_Salt    := (others => 16#0000000000000000#);
      J              : Positive                 := The_Salt'First;
   begin
      for I in PS'Range loop
         PS(I) := Pack(The_Salt(J .. J + BLAKE_512_Word_Bytes - 1), Big_Endian);
         J := J + BLAKE_512_Word_Bytes;
      end loop;
   ...
   function    Unpack_Salt(
      ...
      return   BLAKE_512_Salt
   is
      US             : BLAKE_512_Salt  := (others => 16#00#);
      J              : Positive        := US'First;
   begin
      for I in The_Salt'Range loop
         US(J .. J + BLAKE_512_Word_Bytes - 1) := Unpack(The_Salt(I), Big_Endian);
         J := J + BLAKE_512_Word_Bytes;
      end loop;
   ...
   function    Unpack_State(
      ...
      return   BLAKE_512_Unpacked_State
   is
      US             : BLAKE_512_Unpacked_State := (others => 16#00#);
      J              : Positive                 := US'First;
   begin
      for I in The_State'Range loop
         US(J .. J + BLAKE_512_Word_Bytes - 1) := Unpack(The_State(I), Big_Endian);
         J := J + BLAKE_512_Word_Bytes;
      end loop;
   ...
   is
      ...
      T2             : Eight_Bytes;
   begin
      T1       := X(1 + Natural(BLAKE_512_Sigma(R, I)));
      T2       := BLAKE_512_Constants(1 + Natural(BLAKE_512_Sigma(R, I + 1)));
      V(A)     := V(A) + (T1 xor T2) + V(B);
      V(D)     := Rotate_Right((V(D) xor V(A)), 32);
      V(C)     := V(C) + V(D);
      V(B)     := Rotate_Right((V(B) xor V(C)), 25);

      T1       := X(1 + Natural(BLAKE_512_Sigma(R, I + 1)));
      T2       := BLAKE_512_Constants(1 + Natural(BLAKE_512_Sigma(R, I)));
      V(A)     := V(A) + (T1 xor T2) + V(B);
   ...
   procedure   Transform(
                  ...
                  Null_T         : in     Boolean := False)
   is
      X              : constant BLAKE_512_Packed_Block := Pack_Block(Block);
      V              : BLAKE_512_Working_Vector := (others => 16#0000000000000000#);
   begin
      -- Initialization.

      V( 1 ..  8) := State;

      V( 9)       := Salt(1) xor BLAKE_512_Constants(1);
      V(10)       := Salt(2) xor BLAKE_512_Constants(2);
      V(11)       := Salt(3) xor BLAKE_512_Constants(3);
      V(12)       := Salt(4) xor BLAKE_512_Constants(4);


      if Null_T then
         -- Special case for the last block.

         V(13)    := BLAKE_512_Constants(5);
         V(14)    := BLAKE_512_Constants(6);
         V(15)    := BLAKE_512_Constants(7);
         V(16)    := BLAKE_512_Constants(8);
      else
         V(13)    := BCount(1) xor BLAKE_512_Constants(5);
         V(14)    := BCount(1) xor BLAKE_512_Constants(6);
         V(15)    := BCount(2) xor BLAKE_512_Constants(7);
         V(16)    := BCount(2) xor BLAKE_512_Constants(8);
      end if;

      -- Perform the BLAKE 512 rounds.

      for I in 1 .. BLAKE_512_Rounds loop
         -- Column Step.
   ...
   begin
      P := new BLAKE_512_Digest'(Message_Digest with
                                    Id          => MD_BLAKE_512,
                                    Salt        => BLAKE_512_Default_Packed_Salt,
                                    State       => BLAKE_512_Initial_State,
                                    BCount      => BLAKE_512_Zero_BCount,
                                    BIB         => 0,
                                    Buffer      => (others => 16#00#));

      Private_Initialize_Digest(
         P.all,
         BLAKE_512_State_Bytes,
         BLAKE_512_Block_Bytes,
   ...
   exception
      when others =>
         Raise_Exception(
            ...
            "Error when allocating BLAKE_512_Digest object");
   ...
   begin
      ...
      The_Digest.Salt         := BLAKE_512_Default_Packed_Salt;
      The_Digest.State        := BLAKE_512_Initial_State;
      The_Digest.BCount       := BLAKE_512_Zero_BCount;
      The_Digest.BIB          := 0;
   ...
   is
   begin
      Digest_Start(The_Digest, BLAKE_512_Default_Salt);
   end Digest_Start;
   ...
   is
      ...
      Chunks         : Natural            := TB / BLAKE_512_Block_Bytes;
      New_BIB        : constant Natural   := TB mod BLAKE_512_Block_Bytes;
      I              : Natural            := The_Bytes'First;
      To_Copy        : Natural            := 0;
   begin
      -- Data is processed in chunks of BLAKE_512_Block_Bytes bytes.
      ...
      if Chunks > 0 then
         ...
         if The_Digest.BIB > 0 then
            ...
            To_Copy := BLAKE_512_Block_Bytes - The_Digest.all.BIB;
            The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. BLAKE_512_Block_Bytes) :=
               The_Bytes(I .. I + To_Copy - 1);

            -- Update internal counter.

            The_Digest.all.BCount(1) := The_Digest.all.BCount(1) + BLAKE_512_Block_Bits;
         ...
         while Chunks > 0 loop
            ...
            Transform(
               The_Digest.all.State,
               The_Bytes(I .. I + BLAKE_512_Block_Bytes - 1),
               The_Digest.all.Salt,
               The_Digest.all.BCount,
               False);
   ...
   begin
      ...
      if The_Digest.all.BIB = Pad_Limit then
         ...
         if The_Digest.all.BIB < Pad_Limit then
            ...
            The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. Pad_Limit) := BLAKE_512_Padding(1 .. To_Pad - 1);
            The_Digest.all.Buffer(Pad_Limit + 1) := 16#01#;
         else
            To_Pad := BLAKE_512_Block_Bytes - The_Digest.all.BIB;
            The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. The_Digest.all.Buffer'Last) := BLAKE_512_Padding(1 .. To_Pad);
            Transform(
               The_Digest.all.State,
               The_Digest.all.Buffer,
               The_Digest.all.Salt,
      ...
      The_Digest.all.Buffer(Bit_Counter_Offset .. BLAKE_512_Block_Bytes) := Bit_Counter;
      Transform(
         The_Digest.all.State,
         The_Digest.all.Buffer,
         The_Digest.all.Salt,
