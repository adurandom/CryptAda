...
--------------------------------------------------------------------------------
-- 1. Identification
--    Filename          :  cryptada-digests-message_digests-ripemd_256.adb
...
--------------------------------------------------------------------------------
-- 2. Purpose:
--    Implements the RIPEMD-256 message digest algorithm.
--------------------------------------------------------------------------------
-- 3. Revision history
--    Ver   When     Who   Why
--    ----- -------- ----- -----------------------------------------------------
...
package body CryptAda.Digests.Message_Digests.RIPEMD_256 is
   ...
   --[Bit_Counter_Offset]-------------------------------------------------------
   -- Index of the first byte of the bit counter inside the RIPEMD_256_Block.
   -- The 8 byte counter will occupy the last 8 positions of the last block.
   -----------------------------------------------------------------------------

   Bit_Counter_Offset      : constant Positive := 1 + RIPEMD_256_Block_Bytes - 8;

   --[RIPEMD_256_Block_Words]---------------------------------------------------
   -- Size in words of MD5 block.
   -----------------------------------------------------------------------------

   RIPEMD_256_Block_Words  : constant Positive := RIPEMD_256_Block_Bytes / 4;

   --[RIPEMD_256_Pad]----------------------------------------------------------
   -- Pad block.
   -----------------------------------------------------------------------------

   RIPEMD_256_Pad          : constant RIPEMD_256_Block := (1 => 16#80#, others => 16#00#);

   -----------------------------------------------------------------------------
   --[Types]--------------------------------------------------------------------
   -----------------------------------------------------------------------------

   --[RIPEMD_256_Packed_Block]--------------------------------------------------
   -- Packed block type.
   -----------------------------------------------------------------------------

   subtype RIPEMD_256_Packed_Block is Four_Bytes_Array(1 .. RIPEMD_256_Block_Words);

   --[RIPEMD_256_Unpacked_State]------------------------------------------------
   -- Unpacked state type.
   -----------------------------------------------------------------------------

   subtype RIPEMD_256_Unpacked_State is Byte_Array(1 .. RIPEMD_256_State_Bytes);
   ...
   procedure   Initialize_Object(
                  Object         : access RIPEMD_256_Digest);
   ...
   function    Pack_Block(
                  The_Block      : in     RIPEMD_256_Block)
      return   RIPEMD_256_Packed_Block;
   pragma Inline(Pack_Block);

   --[Unpack_State]-------------------------------------------------------------

   function    Unpack_State(
                  The_State      : in     RIPEMD_256_State)
      return   RIPEMD_256_Unpacked_State;
   ...
   procedure   Transform(
                  State          : in out RIPEMD_256_State;
                  Block          : in     RIPEMD_256_Block);
   ...
   begin
      ...
      Object.all.State     := RIPEMD_256_Initial_State;
      Object.all.BIB       := 0;
   ...
   procedure   Transform(
                  ...
                  Block          : in     RIPEMD_256_Block)
   is
      X              : constant RIPEMD_256_Packed_Block := Pack_Block(Block);
      AA             : Four_Bytes := State(1);
      BB             : Four_Bytes := State(2);
      CC             : Four_Bytes := State(3);
      DD             : Four_Bytes := State(4);
      AAA            : Four_Bytes := State(5);
      BBB            : Four_Bytes := State(6);
      CCC            : Four_Bytes := State(7);
      DDD            : Four_Bytes := State(8);
      T              : Four_Bytes;
   begin
   ...
   is
      P           : RIPEMD_256_Digest_Ptr;
   begin
      P := new RIPEMD_256_Digest'(Message_Digest with
                                    Id          => MD_RIPEMD_256,
                                    State       => RIPEMD_256_Initial_State,
                                    BIB         => 0,
                                    Buffer      => (others => 16#00#));
      Private_Initialize_Digest(
         P.all,
         RIPEMD_256_State_Bytes,
         RIPEMD_256_Block_Bytes,
   ...
   exception
      when others =>
         Raise_Exception(
            ...
            "Error when allocating RIPEMD_256_Digest object");
   ...
   begin
      ...
      The_Digest.State        := RIPEMD_256_Initial_State;
      The_Digest.BIB          := 0;
   ...
   pragma Warnings (On, "formal parameter ""Parameters"" is not referenced");
      -- Parameters is ignored because RIPEMD-256 does not expect any parameter.
   begin
   ...
   is
      ...
      Chunks         : Natural            := TB / RIPEMD_256_Block_Bytes;
      New_BIB        : constant Natural   := TB mod RIPEMD_256_Block_Bytes;
      I              : Natural            := The_Bytes'First;
      To_Copy        : Natural            := 0;
   begin
      -- Data is processed in chunks of RIPEMD_256_Block_Bytes bytes.
      ...
      if Chunks > 0 then
         ...
         if The_Digest.all.BIB > 0 then
            To_Copy := RIPEMD_256_Block_Bytes - The_Digest.all.BIB;
            The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. RIPEMD_256_Block_Bytes) :=
         ...
         while Chunks > 0 loop
            Transform(
               ...
               The_Bytes(I .. I + RIPEMD_256_Block_Bytes - 1));

            -- Update index over The_Bytes, decrease number of chunks.

            I := I + RIPEMD_256_Block_Bytes;
            Chunks := Chunks - 1;
         end loop;
      end if;
   ...
   is
      ...
      To_Pad         : constant Natural := RIPEMD_256_Block_Bytes - The_Digest.BIB;
   begin
      -- Pad buffer.

      if To_Pad > 0 then
         The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. RIPEMD_256_Block_Bytes) :=
            RIPEMD_256_Pad(1 .. To_Pad);
      end if;
      ...
      The_Digest.all.Buffer(Bit_Counter_Offset .. RIPEMD_256_Block_Bytes) := UC(1 .. 8);
      Transform(The_Digest.all.State, The_Digest.all.Buffer);
