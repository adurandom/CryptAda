...
--------------------------------------------------------------------------------
-- 1. Identification
--    Filename          :  cryptada-digests-message_digests-sha_256.adb
...
package body CryptAda.Digests.Message_Digests.SHA_256 is
   ...
   --[Bit_Counter_Offset]-------------------------------------------------------
   -- Index of the first byte of the bit counter inside the SHA_256_Block. The 8
   -- byte counter will occupy the last 8 positions of the last block.
   -----------------------------------------------------------------------------

   Bit_Counter_Offset      : constant Positive := 1 + SHA_256_Block_Bytes - 8;

   --[SHA_256_Pad]--------------------------------------------------------------
   -- Array for padding.
   -----------------------------------------------------------------------------

   SHA_256_Pad             : constant SHA_256_Block := (1 => 16#80#, others => 16#00#);

   --[SHA_256_Block_Words]------------------------------------------------------
   -- Size in words of SHA-256 block.
   -----------------------------------------------------------------------------

   SHA_256_Block_Words     : constant Positive := SHA_256_Block_Bytes / SHA_256_Word_Bytes;

   --[SHA_256_K]----------------------------------------------------------------
   -- SHA-256 constants defined in section 4.2.2 of FIPS-PUB 180-4. According to
   -- that document:
   --
   ...
   SHA_256_K               : constant Four_Bytes_Array(1 .. 64) := (
   ...
   --[SHA_256_Packed_Block]-----------------------------------------------------
   -- Packed block type.
   -----------------------------------------------------------------------------

   subtype SHA_256_Packed_Block is Four_Bytes_Array(1 .. SHA_256_Block_Words);

   --[SHA_256_Unpacked_State]---------------------------------------------------
   -- Unpacked state type.
   -----------------------------------------------------------------------------

   subtype SHA_256_Unpacked_State is Byte_Array(1 .. SHA_256_State_Bytes);
   ...
   procedure   Initialize_Object(
                  Object         : access SHA_256_Digest);
   pragma Inline(Initialize_Object);
   
   --[Pack_Block]---------------------------------------------------------------

   function    Pack_Block(
                  The_Block      : in     SHA_256_Block)
      return   SHA_256_Packed_Block;
   pragma Inline(Pack_Block);

   --[Unpack_State]-------------------------------------------------------------

   function    Unpack_State(
                  The_State      : in     SHA_256_State)
      return   SHA_256_Unpacked_State;
   ...
   procedure   Transform(
                  State          : in out SHA_256_State;
                  Block          : in     SHA_256_Block);
   ...
   begin
      ...
      Object.all.State     := SHA_256_Initial_State;
      Object.all.BIB       := 0;
   ...
   function    Pack_Block(
      ...
      return   SHA_256_Packed_Block
   is
      PB             : SHA_256_Packed_Block;
      J              : Positive := The_Block'First;
   begin
      for I in PB'Range loop
         PB(I) := Pack(The_Block(J .. J + SHA_256_Word_Bytes - 1), Big_Endian);
         J := J + SHA_256_Word_Bytes;
      end loop;
   ...
   function    Unpack_State(
      ...
      return   SHA_256_Unpacked_State
   is
      US             : SHA_256_Unpacked_State;
      J              : Positive := US'First;
   begin
      for I in The_State'Range loop
         US(J .. J + SHA_256_Word_Bytes - 1) := Unpack(The_State(I), Big_Endian);
         J := J + SHA_256_Word_Bytes;
      end loop;
   ...
   procedure   Transform(
                  ...
                  Block          : in     SHA_256_Block)
   is
      T              : SHA_256_State := State;
      X              : constant SHA_256_Packed_Block := Pack_Block(Block);
   ...
   begin
      ...
      W(1 .. SHA_256_Block_Words) := X;

      for I in SHA_256_Block_Words + 1 .. W'Last loop
      ...
      for I in 1 .. 64 loop
         Y := T(8) + Ep_1(T(5)) + Ch(T(5), T(6), T(7)) + SHA_256_K(I) + W(I);
   ...
   is
      P           : SHA_256_Digest_Ptr;
   begin
      P := new SHA_256_Digest'(Message_Digest with
                                 Id          => MD_SHA_256,
                                 State       => SHA_256_Initial_State,
                                 BIB         => 0,
                                 Buffer      => (others => 16#00#));
   ...
   exception
      when others =>
         Raise_Exception(
            ...
            "Error when allocating SHA_256_Digest object");
   ...
   begin
      ...
      The_Digest.State        := SHA_256_Initial_State;
      The_Digest.BIB          := 0;
   ...
   procedure   Digest_Start(
                  The_Digest     : access SHA_256_Digest)
   is
   begin
   ...
   pragma Warnings (On, "formal parameter ""Parameters"" is not referenced");
      -- Parameters is ignored because SHA-256 does not expect any parameter.
   begin
   ...
   is
      ...
      Chunks         : Natural            := TB / SHA_256_Block_Bytes;
      New_BIB        : constant Natural   := TB mod SHA_256_Block_Bytes;
      I              : Natural            := The_Bytes'First;
      To_Copy        : Natural            := 0;
   begin
      -- Data is processed in chunks of SHA_256_Block_Bytes bytes.
      ...
      if Chunks > 0 then
         ...
         if The_Digest.all.BIB > 0 then
            To_Copy := SHA_256_Block_Bytes - The_Digest.all.BIB;
            The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. SHA_256_Block_Bytes) := 
         ...
         while Chunks > 0 loop
            Transform(The_Digest.all.State, The_Bytes(I .. I + SHA_256_Block_Bytes - 1));

            -- Update index over The_Bytes, decrease number of chunks.

            I := I + SHA_256_Block_Bytes;
            Chunks := Chunks - 1;
         end loop;
      end if;
   ...
   is
      ...
      To_Pad         : constant Natural := SHA_256_Block_Bytes - The_Digest.all.BIB;
   begin
      -- Pad message.

      if To_Pad > 0 then
         The_Digest.all.Buffer(The_Digest.all.BIB + 1 .. SHA_256_Block_Bytes) := 
            SHA_256_Pad(1 .. To_Pad);
      end if;
      ...
      The_Digest.all.Buffer(Bit_Counter_Offset .. SHA_256_Block_Bytes) := UC(9 .. 16);
      Transform(The_Digest.all.State, The_Digest.all.Buffer);
...
end CryptAda.Digests.Message_Digests.SHA_256;
