...
--------------------------------------------------------------------------------
-- 1. Identification
--    Filename          :  cryptada-pragmatics-lists-integer_item.adb
...
package body CryptAda.Pragmatics.Lists.Integer_Item is

   -----------------------------------------------------------------------------
   --[Generic instantiations]---------------------------------------------------
   -----------------------------------------------------------------------------

   package IIO is new Ada.Text_IO.Integer_IO(Number);
   ...
   begin
      ...
      if IP.all.Kind = Integer_Item_Kind then
         return Number(IP.all.Integer_Value);
      else
         Raise_Exception(CryptAda_Item_Kind_Error'Identity, "Item is not an integer value");
      end if;
   end Get_Value;
   ...
   begin
      ...
      if IP.all.Kind /= Integer_Item_Kind then
         Raise_Exception(CryptAda_Item_Kind_Error'Identity, "Item is not an integer");
      end if;

      IP.all.Integer_Value := Integer(Value);
   end Replace_Value;
   ...
   begin
      ...
      IP                      := Allocate_Item(Integer_Item_Kind);
      IP.all.Integer_Value    := Integer(Value);
   ...
   begin
      ...
      while I <= L loop
         if IP.all.Kind = Integer_Item_Kind then
            if IP.all.Integer_Value = Integer(Value) then
               return I;
            end if;
         end if;
      ...
      Raise_Exception(CryptAda_Item_Not_Found_Error'Identity, "Not found an integer item with the value provided");
   end Position_By_Value;
   
end CryptAda.Pragmatics.Lists.Integer_Item;