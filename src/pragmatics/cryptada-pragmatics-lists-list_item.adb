...
--------------------------------------------------------------------------------
-- 1. Identification
--    Filename          :  cryptada-pragmatics-lists-list_item.adb
...
package body CryptAda.Pragmatics.Lists.List_Item is
   ...
   begin
      ...
      if IP.all.Kind = List_Item_Kind then
         LRP := Clone_List_Record(IP.all.List_Value);
         Deallocate_List_Record(Value.Outermost);
         Value.Outermost   := LRP;
         Value.Current     := LRP;
      else
         Raise_Exception(CryptAda_Item_Kind_Error'Identity, "Item is not a list value");
      end if;
   end Get_Value;
   ...
   begin
      ...
      if IP.all.Kind /= List_Item_Kind then
         Raise_Exception(CryptAda_Item_Kind_Error'Identity, "Item is not a list");
      end if;

      LRP := Clone_List_Record(Value.Current);
      Deallocate_List_Record(IP.all.List_Value);
      IP.all.List_Value := LRP;
   end Replace_Value;
   ...
   begin
      ...
      IP                := Allocate_Item(List_Item_Kind);
      IP.all.List_Value := Clone_List_Record(Value.Current);
   ...
   begin
      ...
      while I <= L loop
         if IP.all.Kind = List_Item_Kind then
            if Is_Equal(Value.Current, IP.all.List_Value) then
               return I;
            end if;
         end if;
      ...
      Raise_Exception(CryptAda_Item_Not_Found_Error'Identity, "Not found a list item with value provided");
   end Position_By_Value;
   
end CryptAda.Pragmatics.Lists.List_Item;