...
--------------------------------------------------------------------------------
-- 2. Purpose:
--    This package provides the specification of CryptAda lists. CryptAda lists
--    are a linearly ordered sets of data elements called list items. Each list
--    item has an item value. A list item may also have an item name, in which
--    case it is called a named item; if a list item has no item name, it is
--    called an unnamed item.
--
...
--      items.
--
--    Lists can contain list items thus providing an arbitrary nested list
--    structure. Each list object has a reference to the outermost list in the
--    structure and a 'currrent' list which, by default, is the outermost list
--    but can be set to any list in the structure. Most list operations are
--    executed on the 'current' list and not in the 'outermost' list.
--
--    Item names are identifiers that meet the syntax rules for Ada identifiers
--    which are summarized as follows:
--
--    -  First character of an identifier must be a letter.
--    -  Next characters could be any sequence of letters, digits or the '_'
--       character provided that no two or more '_' together are allowed and
--       the '_' could not be the last character in the identifier.
--    -  Identifiers could not be Ada reserved words.
--    -  Identifier comparison is case unsensitive so This_Identifier is equal
--       to THIS_IDENTIFIER.
--
--    This package provides a Identifier type to handle the identifiers.
--
...
--    1.1   20170430 ADD   Moved from Pragmatics to root.
--------------------------------------------------------------------------------
...
package CryptAda.Lists is
   ...
   -- Maximum length of an Identifier text.
   -----------------------------------------------------------------------------
   ...
   --[List_Text]----------------------------------------------------------------
   -- Type for list text representations.
   -----------------------------------------------------------------------------
   ...
   -- identifiers name the values in named lists and could be also values
...
private
   ...
   --[List Items Related Definitions]-------------------------------------------
   -- Next definitions are related to list items.
   -----------------------------------------------------------------------------
   ...
   --[Item List]----------------------------------------------------------------
   -- Generic instantiation of Ada.Containers.Doubly_Linked_List
   -----------------------------------------------------------------------------

   package Item_List_Pkg is new Ada.Containers.Doubly_Linked_Lists(Item_Ptr);

   subtype Item_List is Item_List_Pkg.List;

   subtype Item_List_Cursor is Item_List_Pkg.Cursor;

   --[Item Hash Map]------------------------------------------------------------
   -- Generic instantiation of Ada.Containers.Hashed_Maps
   -----------------------------------------------------------------------------
   ...
   package Item_Hash_Map_Pkg is new Ada.Containers.Indefinite_Hashed_Maps(
                                       Key_Type          => Identifier_Text,
                                       Element_Type      => Item_Ptr,
                                       Hash              => Hash_Identifier,
                                       Equivalent_Keys   => Is_Equal);

   subtype Item_Map is Item_Hash_Map_Pkg.Map;

   subtype Item_Map_Cursor is Item_Hash_Map_Pkg.Cursor;

   --[List Related Definitions]-------------------------------------------------
   -- Next definitions are related to lists.
   -----------------------------------------------------------------------------
   ...
   -- Names             The hash map for item names.
   -----------------------------------------------------------------------------
